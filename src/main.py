...
def smart_read_historical(path: str) -> pd.DataFrame:
    """Lee CSV con delimitador auto y fallback a ';' si venía todo en una columna."""
    try:
...
def parse_tmo_to_seconds(val):
    """Convierte TMO en 'hh:mm[:ss]' o número a segundos (float)."""
    if pd.isna(val):
        return np.nan
...
def load_holidays(csv_path: str) -> set:
    """Carga feriados en set de fechas (date)."""
...
def mark_holidays_index(dt_index, holidays_set: set) -> pd.Series:
    """Marca 1 si la fecha está en feriados, 0 si no."""
...
def add_es_dia_de_pago(df_idx: pd.DataFrame) -> pd.Series:
    """Marca 1 si el día es 1,2,15,16,29,30,31."""
...
def safe_reset_index_ts(df: pd.DataFrame, prefer: str = "ts") -> pd.DataFrame:
    """
    reset_index evitando choque de nombres.
    Si ya existe 'ts', crea 'ts_1', 'ts_2', ...
    """
    name = prefer
    i = 1
...
def main(horizonte_dias: int):
    ...
    # Alias para TMO (si viniera en el principal)
    ...
    # Index temporal robusto (tz aware)
    dfh = ensure_ts(dfh)
    ...
    df_hourly = forecast_120d(
        safe_reset_index_ts(dfh),   # <= evita ValueError: cannot insert <idx_name>, already exists
    ...
    write_hourly_json(
        ...
        calls_col="calls",
        tmo_col="tmo_s",
        agentes_col="agentes_requeridos",
    )
    write_daily_json(
