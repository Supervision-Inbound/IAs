...
def main(horizonte_dias: int):
    ...
    df_tmo_hist_only = None  # <-- Variable para TMO puro (usada en v1)
    if os.path.exists(TMO_HIST_FILE):
        df_tmo = load_historico_tmo(TMO_HIST_FILE)  # index ts
        df_tmo_hist_only = df_tmo.copy()  # <-- Guardamos la data pura (usada en v1)
    ...
    # 4) ffill de columnas clave para evitar NaN en el borde (LÓGICA v1)
    ...
    df_hourly = forecast_120d(
        dfh.reset_index(),
        df_tmo_hist_only.reset_index() if df_tmo_hist_only is not None else None, # <-- Se pasa df_tmo_hist_only
