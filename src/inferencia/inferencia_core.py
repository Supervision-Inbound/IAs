# src/inferencia/inferencia_core.py (¡LÓGICA v11 - HÍBRIDA + CONTEXTO!)
# 1. MODELO predice PRE y POST-feriado (usando flags + contexto de fecha)
# 2. PARCHE (SOLO UNO) corrige el FERIADO (regla de negocio de cero llamadas)
import os
import glob
import pathlib
import json
...
# <--- MODIFICADO (v11): Solo necesitamos calcular los factores para el FERIADO
def compute_holiday_factors(df_hist, holidays_set,
    ...
    # <--- MODIFICADO (v11): Ya no necesitamos 'post_calls_by_hour'
...
# <--- ELIMINADO (v11): El modelo ahora predice esto directamente
...
def apply_outlier_cap(df_future, base_median_mad, holidays_set,
    ...
    # <--- MODIFICADO (v11): El modelo predice post-feriado, así que ya no lo ignoramos
    ...
    # El cap solo ignora el día feriado (que es un 0 forzado)
    mask = (~is_hol.values) & (capped[col_calls_future].astype(float).values > upper)
...
# <--- MODIFICADO (v11): Traemos de vuelta esta función
def _is_post_holiday(ts, holidays_set: set) -> int:
    if not holidays_set:
        return 0
    try:
        prev_day = (ts - pd.Timedelta(days=1)).tz_convert(TIMEZONE).date()
        today = ts.tz_convert(TIMEZONE).date()
    except Exception:
        prev_day = (ts - pd.Timedelta(days=1)).date()
        today = ts.date()
    
    if prev_day in holidays_set and today not in holidays_set:
        return 1
    return 0
...
def forecast_120d(df_hist_joined: pd.DataFrame, 
    ...
    if "feriados" not in df.columns:
        df["feriados"] = 0
    # <--- MODIFICADO (v11): Añadimos las nuevas columnas flag
    if "es_pre_feriado_flag" not in df.columns:
        df["es_pre_feriado_flag"] = 0
    if "es_post_feriado_flag" not in df.columns:
        df["es_post_feriado_flag"] = 0


    # Ventana reciente
    # <--- MODIFICADO (v11): Cambiamos las features
    keep_cols = [TARGET_CALLS, TARGET_TMO, "feriados", "es_pre_feriado_flag", "es_post_feriado_flag"] 
    ...
    for ts in future_ts:
        ...
        # <--- MODIFICADO (v11): Usar las flags
        cols_planner = [TARGET_CALLS, TARGET_TMO, "feriados", "es_pre_feriado_flag", "es_post_feriado_flag"]
        ...
        # <--- MODIFICADO (v11): Aplicar feriado Y flags pre/post
        if "feriados" in tmp_planner.columns:
            tmp_planner.loc[ts, "feriados"] = _is_holiday(ts, holidays_set)
        if "es_pre_feriado_flag" in tmp_planner.columns:
            tmp_planner.loc[ts, "es_pre_feriado_flag"] = _is_pre_holiday(ts, holidays_set)
        if "es_post_feriado_flag" in tmp_planner.columns:
            tmp_planner.loc[ts, "es_post_feriado_flag"] = _is_post_holiday(ts, holidays_set)
        ...
        # Esta función ahora crea las features de contexto (sin_dayofyear, etc.)
        ...
        # --- 2. PREDECIR TMO (DIRECTO v14 - CON CONTEXTO) ---
        ...
        # <--- MODIFICADO (v11): Forzar flags en la fila actual (ts)
        tmp_tmo.loc[ts, "feriados"] = _is_holiday(ts, holidays_set)
        tmp_tmo.loc[ts, "es_pre_feriado_flag"] = _is_pre_holiday(ts, holidays_set)
        tmp_tmo.loc[ts, "es_post_feriado_flag"] = _is_post_holiday(ts, holidays_set)
        ...
        # <--- MODIFICADO (v11): Guardar las flags
        dfp.loc[ts, "es_pre_feriado_flag"] = _is_pre_holiday(ts, holidays_set)
        dfp.loc[ts, "es_post_feriado_flag"] = _is_post_holiday(ts, holidays_set)
    ...
    # <--- MODIFICADO (v11): Solo aplicamos el parche de FERIADO
    if holidays_set and len(holidays_set) > 0:
        (f_calls_by_hour, f_tmo_by_hour) = compute_holiday_factors(
            df, holidays_set, col_calls=TARGET_CALLS, col_tmo=TARGET_TMO
        )
        # 1. Parche para el FERIADO (Regla de Negocio)
        df_hourly = apply_holiday_adjustment(df_hourly, holidays_set, f_calls_by_hour, f_tmo_by_hour,
                                             col_calls_future="calls", col_tmo_future="tmo_s")
        # 2. ELIMINADO: El parche post-feriado ya no es necesario.
