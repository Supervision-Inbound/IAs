...
# IMPORTACIONES LOCALES (Asegúrate de que estas rutas son correctas)
from .features import ensure_ts, add_time_parts 
from .erlang import required_agents, schedule_agents # Asumiendo que estas existen en .erlang

# 💥 ELIMINAMOS LA IMPORTACIÓN DE utils_io para evitar fallos silenciosos en la escritura JSON.
...
# --- Rutas de Artefactos (IMPLEMENTACIÓN FUNCIONAL DE RUTA/CARGA) ---
def _candidate_dirs():
...
# Remplaza estas rutas con las rutas reales si son fijas, o asegúrate que _find_one funcione
PLANNER_MODEL = _find_one(["modelo_planner.keras", "planner*.keras", "*_planner*.keras"]) or "models/modelo_planner.keras"
PLANNER_SCALER = _find_one(["scaler_planner.pkl", "*planner*scaler*.pkl", "*scaler*_planner*.pkl"]) or "models/scaler_planner.pkl"
PLANNER_COLS   = _find_one(["training_columns_planner.json", "*planner*columns*.json"]) or "models/training_columns_planner.json"
...
def _load_cols(path: str):
    if path is None or not os.path.exists(path):
        # ⚠️ Si el archivo no existe, retornamos un set de columnas genérico para evitar fallos de reindex
        print(f"Advertencia: Archivo de columnas no encontrado en {path}. Usando columnas genéricas.")
        return ['dow_0', 'dow_1', 'dow_2', 'dow_3', 'dow_4', 'dow_5', 'dow_6', 'hour_0', 'hour_1', 'hour_23', 'lag_168']
...
def _series_is_holiday(idx, holidays_set: Set[Any]):
    if not holidays_set: return pd.Series(False, index=idx, dtype=bool)
...
def _is_holiday(ts, holidays_set: Set[Any]) -> int:
    if not holidays_set: return 0
    try: d = ts.tz_convert(TIMEZONE).date()
    except Exception: d = ts.date()
    return 1 if d in holidays_set else 0

def compute_holiday_factors(df_hist: pd.DataFrame, holidays_set: Set[Any], col_calls=TARGET_CALLS, col_tmo=TARGET_TMO):
    cols = [col_calls]
    med_hol_tmo = med_nor_tmo = None
    if col_tmo in df_hist.columns: cols.append(col_tmo)
    ...
    factors_calls_by_hour = {
        int(h): _safe_ratio(med_hol_calls.get(h, np.nan), med_nor_calls.get(h, np.nan), fallback=global_calls_factor)
        for h in range(24)
    }
    if med_hol_tmo is not None:
        factors_tmo_by_hour = {
            int(h): _safe_ratio(med_hol_tmo.get(h, np.nan), med_nor_tmo.get(h, np.nan), fallback=global_tmo_factor)
            for h in range(24)
        }
...
def apply_holiday_adjustment(df_future: pd.DataFrame, holidays_set: Set[Any], factors_calls_by_hour: Dict[int, float], factors_tmo_by_hour: Dict[int, float], col_calls_future="calls", col_tmo_future="tmo_s"):
...
def _baseline_median_mad(df_hist: pd.DataFrame, col=TARGET_CALLS):
...
def apply_outlier_cap(df_future: pd.DataFrame, base_median_mad: pd.DataFrame, holidays_set: Set[Any], col_calls_future="calls", k_weekday=K_WEEKDAY, k_weekend=K_WEEKEND):
...
def generate_features(df: pd.DataFrame, target_calls: str, target_tmo: str, feriados_col: str):
    d = add_time_parts(df.copy())
    if feriados_col in d.columns:
    ...
    for lag in [24, 48, 72, 168]: d[f'lag_{lag}'] = s_calls.shift(lag)
    for window in [24, 72, 168]: d[f'ma_{window}'] = s_calls.rolling(window, min_periods=1).mean()
    ...
    # Usaremos el target_calls como proxy para "contestadas" si no existe una columna específica
    s_contest = d[target_calls] 
...
def forecast_120d(df_hist_joined: pd.DataFrame, 
                  horizon_days: int = 120, holidays_set: Set[Any] | None = None):
    ...
    try:
        ...
        print(f"ERROR: Fallo al cargar artefactos del modelo. Verifique rutas: {e}")
        return pd.DataFrame()

    # 2. Preparar Dataframe Histórico (dfp)
    df = ensure_ts(df_hist_joined)
    
    # Manejo de Zona Horaria (Refuerzo)
    if not isinstance(df.index, pd.DatetimeIndex):
        df.index = pd.to_datetime(df.index, errors="coerce", utc=True).tz_convert(TIMEZONE)
    else:
        try: df.index = df.index.tz_convert(TIMEZONE)
        except: df.index = df.index.tz_localize("UTC", ambiguous='infer', nonexistent='shift-forward').tz_convert(TIMEZONE)
    ...
    # REFUERZO EXTREMO: Limpieza de duplicados antes de dummies/reindex
    if dfp.index.duplicated().any():
        dfp = dfp[~dfp.index.duplicated(keep='last')]
    
    # Generar DUMMIES y Reindexar
    dfp = add_time_parts(dfp)
    
    # Limpieza Adicional de Índice (Por si acaso)
    if dfp.index.duplicated().any():
        dfp = dfp[~dfp.index.duplicated(keep='last')]
    ...
    for i in range(horizon_days):
        ...
        # Preparar Inputs LSTM y Predicción
        input_features_pl = input_df[cols_pl].values 
        input_scaled_pl = sc_pl.transform(input_features_pl)
        input_lstm_pl = input_scaled_pl.reshape((1, LOOKBACK_STEPS, len(cols_pl)))
        input_features_tmo = input_df[cols_tmo].values
    ...
    # ===== Salida, Ajustes y Erlang =====
    df_hourly = pd.concat(future_predictions)
    df_hourly = df_hourly.rename(columns={TARGET_CALLS: "calls", TARGET_TMO: "tmo_s"})
    df_hourly["calls"] = np.round(df_hourly["calls"].replace([np.inf, -np.inf], np.nan).fillna(0.0)).astype(int)
    df_hourly["tmo_s"] = np.round(df_hourly["tmo_s"].replace([np.inf, -np.inf], np.nan).fillna(0.0)).astype(int)

    if holidays_set and len(holidays_set) > 0:
        (f_calls_by_hour, f_tmo_by_hour, _gc, _gt) = compute_holiday_factors(df, holidays_set, col_calls=TARGET_CALLS, col_tmo=TARGET_TMO)
    ...
    for ts in df_hourly.index:
        # Asumiendo que required_agents está definido o importado desde .erlang
        a, _ = required_agents(float(df_hourly.at[ts, "calls"]), float(df_hourly.at[ts, "tmo_s"])) 
        df_hourly.at[ts, "agents_prod"] = int(a)
    # Asumiendo que schedule_agents está definido o importado desde .erlang
    df_hourly["agents_sched"] = df_hourly["agents_prod"].apply(schedule_agents) 

    # =========================================================================
    # 💥 MODIFICACIÓN CRÍTICA: Implementación de la escritura JSON (directo con Pandas)
    # =========================================================================
    
    # --- 1. Escritura Horaria (prediccion_horaria.json) ---
    try:
        df_hourly_out = df_hourly[["calls", "tmo_s", "agents_sched"]].copy()
        df_hourly_out = df_hourly_out.reset_index().rename(columns={'index': 'ts'})
        df_hourly_out['ts'] = df_hourly_out['ts'].dt.strftime('%Y-%m-%dT%H:%M:%S')
        
        hourly_path = f"{PUBLIC_DIR}/prediccion_horaria.json"
        
        df_hourly_out.to_json(hourly_path, orient='records', date_format='iso', indent=4)
        print(f"Predicción horaria escrita con éxito en: {hourly_path}")

    except Exception as e:
        print(f"ERROR: Fallo al escribir prediccion_horaria.json: {e}")
    
    # --- 2. Escritura Diaria (prediccion_diaria.json) ---
    try:
        # Lógica de Agregación Diaria
        df_daily = df_hourly[["calls", "agents_sched"]].resample('D').sum()
        # Cálculo de TMO promedio ponderado (calls * tmo / calls totales)
        df_daily["tmo_s"] = df_hourly.loc[df_hourly["calls"] > 0, ["calls", "tmo_s"]].prod(axis=1).resample('D').sum() / df_daily["calls"]
        df_daily["tmo_s"] = df_daily["tmo_s"].replace([np.inf, -np.inf], np.nan).fillna(0).round().astype(int)
        # Promedio de agentes
        df_daily["agents_sched"] = df_hourly["agents_sched"].resample('D').mean().round().astype(int)
        
        df_daily_out = df_daily.reset_index().rename(columns={'index': 'ts'})
        df_daily_out['ts'] = df_daily_out['ts'].dt.strftime('%Y-%m-%d')
        
        daily_path = f"{PUBLIC_DIR}/prediccion_diaria.json"
        
        df_daily_out.to_json(daily_path, orient='records', date_format='iso', indent=4)
        print(f"Predicción diaria escrita con éxito en: {daily_path}")

    except Exception as e:
        print(f"ERROR: Fallo al escribir prediccion_diaria.json: {e}")
