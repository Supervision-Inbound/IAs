# src/inferencia/inferencia_core.py (¡LÓGICA v8 - MODELO APRENDE PRE/POST!)
import os
import glob
import pathlib
import json
...
# <--- ELIMINADO (v8): Ya no necesitamos el factor JSON
...
# <--- ELIMINADO (v8): Ya no cargamos factores
...
# <--- ELIMINADO (v8): La lógica de factores ya no es necesaria
# def compute_holiday_factors(...): ...

# <--- ELIMINADO (v8): El modelo predice esto directamente
...
def apply_outlier_cap(df_future, base_median_mad, holidays_set,
    ...
    # (Esta función se mantiene, es una regla de negocio separada)
    if df_future.empty:
        return df_future
    ...
    # <--- MODIFICADO (v8): Ya no nos saltamos post-feriado
...
# <--- NUEVO (v8): Función para detectar el día POSTERIOR al feriado
...
def forecast_120d(df_hist_joined: pd.DataFrame, 
    ...
    if "feriados" not in df.columns:
        df["feriados"] = 0
    # <--- MODIFICADO (v8): Añadimos las nuevas columnas flag
    ...
    # Ventana reciente
    # <--- MODIFICADO (v8): Cambiamos las features
    ...
    for ts in future_ts:
        ...
        # <--- MODIFICADO (v8): Aplicar feriado Y flags pre/post
        ...
        # --- 2. PREDECIR TMO (DIRECTO v11 - CON FLAGS PRE/POST) ---
        ...
        # <--- MODIFICADO (v8): Forzar flags en la fila actual (ts)
        ...
        # <--- MODIFICADO (v8): Guardar las flags
    ...
    # <--- ELIMINADO (v8): El modelo ahora predice esto directamente.
