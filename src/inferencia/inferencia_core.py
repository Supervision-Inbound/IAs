# src/inferencia/inferencia_core.py (¡LÓGICA v9 - HÍBRIDA!)
# 1. MODELO predice PRE-feriado (usando 'es_pre_feriado_flag')
# 2. PARCHE corrige FERIADO y POST-feriado (con la lógica v7.1)
import os
import glob
import pathlib
import json
...
# <--- MODIFICADO (v9): Traemos de vuelta los parches, con la corrección v7.1
def compute_holiday_factors(df_hist, holidays_set,
    ...
    # Esta es la corrección v7.1 que permite valles post-feriado
    post_calls_by_hour = {
        int(h): _safe_ratio(med_post_calls.get(h, np.nan),
                            med_nor_calls.get(h, np.nan),
                            fallback=1.0) # Fallback neutro
        for h in range(24)
    }
    post_calls_by_hour = {h: float(np.clip(v, 0.50, 1.50)) for h, v in post_calls_by_hour.items()} # Permite valles
...
def apply_outlier_cap(df_future, base_median_mad, holidays_set,
    ...
    # <--- MODIFICADO (v9): El cap de outliers SÍ debe ignorar el post-feriado
...
# <--- ELIMINADO (v9): El 'post_holiday' lo maneja el parche, no el bucle.
# def _is_post_holiday(...): ...
...
def forecast_120d(df_hist_joined: pd.DataFrame, 
    ...
    if "feriados" not in df.columns:
        df["feriados"] = 0
    # <--- MODIFICADO (v9): Solo 'es_pre_feriado_flag'
    if "es_pre_feriado_flag" not in df.columns:
        df["es_pre_feriado_flag"] = 0

    # Ventana reciente
    # <--- MODIFICADO (v9): Solo 'es_pre_feriado_flag'
    keep_cols = [TARGET_CALLS, TARGET_TMO, "feriados", "es_pre_feriado_flag"] 
    ...
    for ts in future_ts:
        ...
        # <--- MODIFICADO (v9): Solo 'es_pre_feriado_flag'
        cols_planner = [TARGET_CALLS, TARGET_TMO, "feriados", "es_pre_feriado_flag"]
        ...
        # <--- MODIFICADO (v9): Aplicar feriado Y flag PRE
        ...
        # --- 2. PREDECIR TMO (DIRECTO v12 - SOLO PRE-FLAG) ---
        ...
        # <--- MODIFICADO (v9): Forzar flags en la fila actual (ts)
    ...
    # <--- MODIFICADO (v9): Traemos de vuelta los parches
