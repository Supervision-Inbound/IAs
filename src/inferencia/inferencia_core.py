...
def _series_is_holiday(idx: pd.DatetimeIndex, holidays_set: set) -> pd.Series:
    ...
    Normaliza a TIMEZONE para comparar por fecha local.
    """
    if idx.tz is None:
...
def forecast_120d(df_hist_joined: pd.DataFrame,
    ...
    # === 2) Histórico TMO puro (archivo TMO original) ===
    ...
    # Añadir opcionales de TMO si existen (no obligatorio pero útiles como features)
    ...
    if TARGET_TMO not in dfp.columns:
        ...
        if has_all:
            qc = pd.to_numeric(dfp["q_llamadas_comercial"], errors="coerce")
            qt = pd.to_numeric(dfp["q_llamadas_tecnico"], errors="coerce")
            tc = pd.to_numeric(dfp["tmo_comercial"], errors="coerce")
            tt = pd.to_numeric(dfp["tmo_tecnico"], errors="coerce")
            den = (qc.fillna(0) + qt.fillna(0)).replace(0, np.nan)
            num = (tc.fillna(0) * qc.fillna(0)) + (tt.fillna(0) * qt.fillna(0))
            dfp[TARGET_TMO] = (num / den)
        else:
