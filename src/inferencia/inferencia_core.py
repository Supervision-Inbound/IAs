...
def forecast_120d(df_hist_joined: pd.DataFrame, 
    ...
    # Saneamiento de NaNs/infs ANTES de convertir a int (PARA CALLS)
    calls_series = dfp.loc[future_ts, TARGET_CALLS]
    calls_series_clean = calls_series.replace([np.inf, -np.inf], np.nan).fillna(0.0)
    df_hourly["calls"] = np.round(calls_series_clean).astype(int)
    # --- FIN DE LA CORRECCIÓN ---
    
    # Saneamiento de NaNs/infs ANTES de convertir a int (PARA TMO)
